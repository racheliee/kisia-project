...
class DynamicDetector:
    # 분석할 폴더 경로 설정
    def __filepath(self): 
        base_folder_path = "./dynamic_temps/"
...
#         print("CSV 파일 저장 중 에러 발생:", e)
    ...
    def __preprocess(self):
        ...
        for file in self.files:
            if file.endswith(".js"):  # JavaScript 파일만 분석
                ...
                # Jalangi2 명령어 실행
                command = f"node ../jalangi2/src/js/commands/jalangi.js --inlineIID --inlineSource --analysis ./Myanalysis_v1.js {file_path}"
    ...
    def __predict(self):
        model_path = './model_dynamic.pkl'  # 모델 파일 경로
        csv_path = './analysis_results.csv'  # 분석할 CSV 파일 경로
        # .pkl 파일에서 모델을 로드
        try:
        ...
        for row in data:
            ...
            confidence = max(model.predict_proba(row_reshaped)[0])  # confidence 값
            predictions.append({
